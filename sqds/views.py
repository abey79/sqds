...
class UnitView(MetadataMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .prefetch_related(
                "mod_set",
                "medal_set",
                "unit__stat_medal_rule_set",
                "unit__zeta_medal_rule_set",
                "unit__zeta_medal_rule_set__skill",
            )
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({v: None for v in slot_to_mod.values()})
        for mod in player_unit.mod_set.all():
            context[slot_to_mod[mod.slot]] = mod
        context["medals"] = player_unit.medals()
        return context

    def get_meta_title(self, context=None):
        player_unit = context["player_unit"]
        return "{}'s {}".format(player_unit.player.name, player_unit.unit.name)
...
class GuildView(MetadataMixin, SingleTableMixin, FilterView):
    ...
    def get_queryset(self):
        qs = (
            self.model.objects.filter(guild__api_id=self.kwargs["api_id"])
            .annotate_stats()
            .annotate_faction_gp()
    ...
    def get_meta_description(self, **kwargs):
        return "{} players, {} GP".format(
            self.guild.player_count, format_large_int(self.guild.gp)
        )
...
class UnitsFilter(FilterSet):
    unit = ChoiceFilter(
        field_name="unit",
        choices=Unit.objects.values_list("id", "name").order_by(Lower("name")),
    )
    category = ChoiceFilter(
        field_name="unit__categories",
        choices=Category.objects.values_list("id", "name").order_by(Lower("name")),
    )
...
class GuildUnitsView(MetadataMixin, SingleTableMixin, FilterView):
    ...
    def get_queryset(self):
        return (
            self.model.objects.filter(player__guild__api_id=self.kwargs["api_id"])
            .annotate_stats()
            .select_related("unit", "player")
        )
...
class GuildComparisonUnitsView(MetadataMixin, SingleTableMixin, FilterView):
    ...
    def get_meta_title(self, **kwargs):
        return format_html(
            "Guild comparison units: {} vs. {}", self.guild1.name, self.guild2.name
        )
...
def player_unregister_me(request):
    if "sqds_my_ally_code" in request.COOKIES:
        response = redirect(
            "sqds:player", ally_code=request.COOKIES["sqds_my_ally_code"]
        )
    else:
...
class SinglePlayerView(MetadataMixin, SingleTableMixin, FilterView):
    ...
    def get_queryset(self):
        return (
            self.model.objects.filter(player__ally_code=self.kwargs["ally_code"])
            .annotate_stats()
            .select_related("unit", "player")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["ga_pools"] = GAPool.objects.filter(focus_player=self.player).order_by(
            "-created"
        )[:10]
        return context
...
class PlayerCompareView(MetadataMixin, TemplateView):
    ...
    def setup(self, request, *args, **kwargs):
        super().setup(request, *args, **kwargs)
        Player.objects.ensure_exist(
            [self.kwargs["ally_code1"], self.kwargs["ally_code2"]]
        )

        qs = (
            Player.objects.filter(
                ally_code__in=[self.kwargs["ally_code1"], self.kwargs["ally_code2"]]
            )
            .annotate_stats()
            .annotate_faction_gp()
        ...
        id_to_name = {
            v["api_id"]: v["name"] for v in Unit.objects.values("api_id", "name")
        }
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            context["mod_speed_graph"] = (
                "Mod speed graph could not be generated due " "to an error."
            )
        try:
            context["gp_analysis_graph"] = self.generate_gp_analysis_graph()
        except IndexError:
            context["gp_analysis_graph"] = (
                "GP analysis graph could not be generated " "due to an error."
            )
        return context
    ...
    def get_meta_description(self, context=None):
        format_string = (
            "Comparision between {} (ally code: {}) " "and {} (ally code: {})"
        )
        return format_string.format(
            self.player1.name,
    ...
    def generate_mod_speed_graph(self):
        ...
        for player in [self.player1, self.player2]:
            qs = (
                PlayerUnit.objects.filter(player__name=player)
                .order_by("-mod_speed")
        ...
        layout = go.Layout(
            xaxis={"title": "Characters, sorted by decreasing mod speed"},
    ...
    def generate_gp_analysis_graph(self):
        ...
        traces = [
            go.Scatter(y=gp - mean_gp, name=player.name)
            for gp, player in [(gp1, self.player1), (gp2, self.player2)]
        ]

        layout = go.Layout(
            xaxis={"title": "Characters, sorted by decreasing GP"},
...
class PlayerCompareUnitsView(MetadataMixin, SingleTableMixin, FilterView):
    ...
    def get_meta_title(self, **kwargs):
        return "Player unit comparison: {} vs. {}".format(
    ...
    def get_meta_description(self, context=None):
        sort_string = str(context["table"].order_by)
        format_string = (
            "{} characters, ordered by {}, from {} (ally code: {}) "
            "and {} (ally code: {})"
        )
        return format_string.format(
...
class AllPlayerUnitsFilter(FilterSet):
    player = ChoiceFilter(
        choices=Player.objects.values_list("id", "name").order_by(Lower("name"))
    )
    category = ChoiceFilter(
        field_name="unit__categories",
        choices=Category.objects.values_list("id", "name").order_by(Lower("name")),
